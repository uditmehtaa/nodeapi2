...
const updatestudentdata=  async(req,res) =>{
    try{
         const db=await database.connectDB();
    const result = await db.collection("Student").updateOne({id:parseInt(req.params.name)}, { $set: req.body  });
   ...
   const deletestudentdata= async(req,res) => {
    try{
         const db=await database.connectDB();
    const result = await db.collection("Student").deleteOne({id:parseInt(req.params.name)});
